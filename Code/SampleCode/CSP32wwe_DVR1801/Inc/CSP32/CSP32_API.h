 ...
 *---------------------------------------------------------------------------------------*/
#define CSP32_KERNEL_RAM_SIZE                   (9 * 1024)
...
{	
  int16_t *Csp32PcmBufPtr;
	...
	int16_t *Csp32PcmBufStartAddress;
  uint8_t CspKernelRam[CSP32_KERNEL_RAM_SIZE];
} CSP32_WORKING_RAM;

typedef struct _CSP32_PCM_BUFFER
{		
...
CSP_HANDLE CSP32_Init(uint8_t *BackgroundModelAddr, uint8_t *CmdModelAddr, int32_t MaxTime, uint8_t *RejModelAddr, CSP32_WORKING_RAM *Csp32WorkingRam, int32_t Csp32KernelRamSize, void *Csp32PcmBuffer);
