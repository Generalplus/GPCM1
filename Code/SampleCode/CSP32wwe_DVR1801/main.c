...
#if defined(__CC_ARM)
__align(4) uint8_t Csp32Dvr1801WorkingRam[sizeof(CSP32_WORKING_RAM)];
__align(4) CSP32_PCM_BUFFER Csp32PcmBuffer;
#elif defined(__GNUC__)
__attribute__ ((aligned (4))) uint8_t Csp32Dvr1801WorkingRam[sizeof(CSP32_WORKING_RAM)];
...
int main(void)
{
  ...
  MODIFY_REG(GPIOA->OBUF, 0x0000FF00, 0);
	...
	while((CSP32Handle = CSP32_User_Init((CSP32_WORKING_RAM*)Csp32Dvr1801WorkingRam, &Csp32PcmBuffer)) == NULL)
	{
	...
	while(1)
	{
      ...
      GPIOA->OBUF &= ~BIT15;
		  //GPIOA->OBUF = CspResult + 1;
			...
			SACM_DVR1801_Initial((SACM_DVR1801_WORKING_RAM*)Csp32Dvr1801WorkingRam, (SACM_DVR1801_TEMP_RAM*)&Csp32Dvr1801WorkingRam[sizeof(SACM_DVR1801_WORKING_RAM)], (SACM_DVR1801_PCM_BUFFER*)&Csp32Dvr1801WorkingRam[(sizeof(SACM_DVR1801_WORKING_RAM) + sizeof(SACM_DVR1801_TEMP_RAM))]);
			SACM_DVR1801_Play(DVR1801_User_GetA1801StartAddr(CspResult + 1), DVR1801_DAC_CH0, DVR1801_AUTO_RAMP_UP + DVR1801_AUTO_RAMP_DOWN);
		  while((SACM_DVR1801_GetStatus() & DVR1801_PLAY_FLAG) != 0)
		  {
		    SACM_DVR1801_ServiceLoop();
			  WDT_Clear();
		  }

	    CSP32Handle = CSP32_User_Init((CSP32_WORKING_RAM*)Csp32Dvr1801WorkingRam, &Csp32PcmBuffer);
			CSP32_Start(CSP32Handle);
			GPIOA->OBUF |= BIT15;
    }
