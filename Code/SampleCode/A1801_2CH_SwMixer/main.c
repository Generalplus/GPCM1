...
__align(4) SACM_MIXER_CH1_WORKING_RAM	MixerCh1WorkingRam;
__align(4) int8_t MicCh1PcmBuffer[(2 * 2 * MIXER_CH1_FRAME_SIZE)];
  ...
  while(1)
	{
	  ...
	  switch(ScanedKey)
        ...
        break;
			...
			case 0x02:                                           // IOA1+VDD: Play
        if(((SACM_A1801_GetStatus() & A1801_PLAY_FLAG) == 0) && ((SACM_A1801_CH2_GetStatus() & A1801_PLAY_FLAG) == 0))
        {
          SACM_MIXER_Ch1_Play(MIXER_CH1_DAC_CH1 , MIXER_CH1_AUTO_RAMP_UP + MIXER_CH1_AUTO_RAMP_DOWN);
        }
				SACM_A1801_Stop();
