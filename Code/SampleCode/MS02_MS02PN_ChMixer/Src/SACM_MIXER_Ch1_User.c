  ...
  DacDataTemp1 = DAC->DAC_CH1_DMA_DATA0 & 0xFFC0;
	...
	do
	{
	  ...
	  CLEAR_BIT(DAC->CTRL, DAC_CTRL_DAC_CH1_DMA_ENABLE);                // DAC_Ch1 DMA request disabled.
		TIMER_Close(mMicDacTimerHandle);
...
{
	// mMicDsAdcDmaHandle = DMA0;
	mMicDacCh1DmaHandle = DMA2;
...
void MIXER_Ch1_CB_StartPlay(const SACM_MIXER_CH1_WORKING_RAM *MixerCh1WorkingRam)
{
  ...
  if((SACM_MIXER_Ch1_GetStatus() & MIXER_CH1_ENABLE_DAC_CH0_FLAG) == 0)
  {
        CLEAR_BIT(DAC->CTRL, DAC_CTRL_DAC_CH1_DMA_ENABLE);                // DAC_Ch1 DMA request disabled.
        TIMER_Close(mMicDacTimerHandle);
        while(DMA_CheckBusy(mMicDacCh1DmaHandle) != 0);
...
void MIXER_Ch1_CB_VoiceProcess(const SACM_MIXER_CH1_WORKING_RAM *MixerCh1WorkingRam, int16_t *DstBufAddr, int8_t *SrcBufAddr)
{
  ...
  if(((SACM_MS02_GetStatus() & MS02_PLAY_FLAG) != 0) && (SACM_MS02_GetStatus() & MS02_PAUSE_FLAG) == 0)
	{
    if((SACM_MS02_GetStatus() & (MS02_DECODE_END_FLAG | MS02_STOP_FLAG)) != 0 )
    {
	...
	{
    if((SACM_MS02PN_GetStatus() & (MS02_DECODE_END_FLAG | MS02_STOP_FLAG)) != 0)
    {
      SACM_MS02PN_Stop();
      mMS02PNStatus = 0;
  ...
  APP_SwMixer_3ch(SwMixerCh2Ptr, SwMixerCh2Ptr, DstBufAddr, DstBufAddr, MIXER_CH1_FRAME_SIZE);
}
