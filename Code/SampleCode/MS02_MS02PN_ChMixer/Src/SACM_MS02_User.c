 ...
 *---------------------------------------------------------------------------------------*/
// extern int16_t MicCh1PcmBuffer[2*MS02_OUT_BUFFER_SIZE];
...
{
	mMS02DacCh0DmaHandle = DMA0;
	mMS02DacCh1DmaHandle = DMA2;
...
void MS02_CB_StartPlay(const SACM_MS02_WORKING_RAM *MS02WorkingRam)
{
	int16_t iCount;
	/*
   * Clear DacOut A/B buffer
   */
  /*
	for(iCount = 0; iCount < (2 * MS02_OUT_BUFFER_SIZE); iCount++)		// Kim add 2021.10.14
  {
		MicCh1PcmBuffer[iCount] = 0x0000;
	}
	*/
}
