 ...
 */
/*
uint8_t MS02AutoRampUpDown(uint16_t mMS02PNStatus)
{
	DacAutoRampDown(mMS02PNStatus);
...
{
	mMS02PNDacCh0DmaHandle = DMA0;
	mMS02PNDacCh1DmaHandle = DMA2;
...
{
/*
	if(((SACM_MS02PN_GetStatus() & MS02_PLAY_FLAG) != 0) && ((SACM_MS02_GetStatus() & MS02_PLAY_FLAG) == 0))
  {
	  if((SACM_MS02PN_GetStatus() & MS02_ENABLE_DAC_CH0_FLAG) != 0)
	  {
...
{
  // SET_BIT(GPIOA->OBUF, BIT8);													 // IOA[8] Kim CPU loading test 2021.06.30
  SACM_MS02PN_DecodeProcess(DstBufAddr, OutBufSize);
  // CLEAR_BIT(GPIOA->OBUF, BIT8);													 // IOA[8] Kim CPU loading test 2021.06.30
