...
void SPI_Flash_Open()
{	
	#if SPI_OUTPUT_SEL == SPI0_IOA9_12
	/*
  ...
  mSpiHandle = SPI0;
	...
	#if SPI_OUTPUT_SEL == SPI1_IOA0_3
	/*
	 * GPIOA[0](CLK): output low
	 * GPIOA[1](MOSI) : output low
	 * GPIOA[2](CS) : output high	 
	 * GPIOA[3](MISO) : input floating
	 */
	GPIO_SetMode(GPIOA, BIT3, GPIO_MODE_FLOATING);
	GPIO_SetMode(GPIOA, (BIT0 | BIT1 | BIT2), GPIO_MODE_OUTPUT);	
	GPIOA->OBUF &= ~(BIT0 | BIT1);
	GPIOA->OBUF |= BIT2;	
	...
	MODIFY_REG(IOFUNC->CTRL0, GPIOFUNC_CTRL0_SPI1_IOSEL_MSK, GPIOFUNC_CTRL0_SPI1_IOA0_3);	
  mSpiHandle = SPI1;
	mSpiCsPin = &GPIOA_OBIT->OBIT02;
	#endif	

	#if SPI_OUTPUT_SEL == SPI1_IOA18_21
	/*
	 * GPIOA[18](CLK): output low
	 * GPIOA[19](MOSI) : output low
	 * GPIOA[20](CS) : output high	 
	 * GPIOA[21](MISO) : input floating
	 */
	GPIO_SetMode(GPIOA, BIT21, GPIO_MODE_FLOATING);
	GPIO_SetMode(GPIOA, (BIT18 | BIT19 | BIT20), GPIO_MODE_OUTPUT);	
	GPIOA->OBUF &= ~(BIT18 | BIT19);
	GPIOA->OBUF |= BIT20;		
	
	SET_BIT(CLOCK->APBCKEN, CLOCK_APBCKEN_SPI1_CLK_ENABLE);                                          // APB SPI1 Clock enable
	MODIFY_REG(IOFUNC->CTRL0, GPIOFUNC_CTRL0_SPI1_HWCS_EN_MSK, GPIOFUNC_CTRL0_SPI1_HWCS_DISABLE);    // SPI1 S/W CS
	MODIFY_REG(IOFUNC->CTRL0, GPIOFUNC_CTRL0_SPI1_IOSEL_MSK, GPIOFUNC_CTRL0_SPI1_IOA18_21);	
  mSpiHandle = SPI1;
	mSpiCsPin = &GPIOA_OBIT->OBIT20;
	#endif		
	
	mSpiHandle->CTRL = SPI_CTRL_MASTER_MODE | SPI_CTRL_CLK_SEL_PCLK_DIV_16 | SPI_CTRL_SPI_ENABLE | SPI_CTRL_MAUT_RW_EN_ENABLE;
...
void SPI_Flash_Close()
{
  if(mSpiHandle != NULL)
	...
	#if (SPI_OUTPUT_SEL == SPI0_IOA9_12) || (SPI_OUTPUT_SEL == SPI0_IOB1_4)
	CLEAR_BIT(CLOCK->APBCKEN, CLOCK_APBCKEN_SPI0_CLK_ENABLE);          // APB SPI0 clock disable
	#endif	
	
	#if (SPI_OUTPUT_SEL == SPI1_IOA0_3) || (SPI_OUTPUT_SEL == SPI1_IOA18_21)
...
{
  MODIFY_REG(mSpiHandle->CTRL, SPI_CTRL_CLK_SEL_PCLK_DIV_MSK, SpiClkDiv);	
}
