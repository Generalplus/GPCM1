...
uint32_t *pTimeBaseTest = (uint32_t *)0x50005020;
/*---------------------------------------------------------------------------------------
 * Subroutine Area
 *---------------------------------------------------------------------------------------*/
...
void TIMEBASE_Open(uint32_t SelSys32K)
{
	...
	SET_BIT(CLOCK->APBCKEN, CLOCK_APBCKEN_CTS_TB_CLK_ENABLE);                              // TimeBase clock enable
  ...
  *pTimeBaseTest &= 0x00000003;
    ...
    while((CLOCK->CLKSTS & CLOCK_CLKSTS_IOSC32K_RDY) == 0);                              // Wait Iosc32K stable
	  MODIFY_REG(ACU->APAD_CTRL, ACU_APAD_X32K_PAD_EN_MSK, ACU_APAD_X32K_PAD_DISABLE);     // IOA[14:13] as GPIO
	  MODIFY_REG(ACU->X32K_CTRL, ACU_X32K_CTRL_X32K_EN_MSK, ACU_X32K_CTRL_X32K_DISABLE);	 // XTAL32K	disabled
	}
	else
	{
    MODIFY_REG(ACU->APAD_CTRL, ACU_APAD_X32K_PAD_EN_MSK, ACU_APAD_X32K_PAD_ENABLE);		   // IOA[14:13] as XTAL32K pins.
    ...
    MODIFY_REG(ACU->I32K_CTRL, ACU_I32K_CTRL_I32K_EN_MSK, ACU_I32K_CTRL_I32K_DISABLE);	   // XTAL32K	disabled
	}
...
void TIMEBASE_Close(void)
{
  ...
  MODIFY_REG(SMU->SYSLOCK, SMU_SYSLOCK_UNLOCK_KEY_MSK, 0);	                             // SMU lock
	...
	TIMEBASE->CTRL = 0;
	CLEAR_BIT(CLOCK->APBCKEN, CLOCK_APBCKEN_CTS_TB_CLK_ENABLE);                            // TimeBase clock disable
}
