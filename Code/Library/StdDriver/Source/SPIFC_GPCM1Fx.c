 ...
 *   Oct. 23, 2024
 * @Abstract:
 *   1. 2024.10.23 Add the Release power down command.
 *
 **************************************************************************************************/
...
#define SPI_FLASH_RDP                  (0xAB)     // Release power down mode


/*---------------------------------------------------------------------------------------
 * Table Declaration Area
 *---------------------------------------------------------------------------------------*/
const uint8_t T_GPLibTag_0100160F[16] =
{
	0x67, 0x50, 0x74, 0x41, 0x47,
	0x01,
	0x0F, 0x16, 0x00, 0x01,
	0x00, 0x02, 0x00, 0x01,
	0xDD, 0x01
...
const uint8_t T_SPIFlash_REMS[] =             // DeviceID, ManufacturerID, RDSR3, RDSR2, WRSR3, WRSR2, QEbit
{
	...
	0x13, 0x5E, 0x00, 0x00, 0x00, 0x00, 0x00,		// XC25D80A
...
int32_t SPIFC_TimingFineTune()
{
	...
	else
	{
		...
		PllClkFreq = (ACU->PLL_CTRL & ACU_PLL_CTRL_CLK_SEL_MSK);
		MODIFY_REG(SMU->SYSLOCK, SMU_SYSLOCK_UNLOCK_KEY_MSK, 0);	                             // SMU lock
		PLL_ChangeClk(PLL_24D576M);

		Cache_Disable();
		ReadDataStartAddr = (uint32_t*) SPIFC_CHECK_SUM_START_ADDRESS;
		CheckSumValue = 0;
	...
	Cache_Disable();
	for(ClkEdgeSelTemp1 = 0; ClkEdgeSelTemp1 <= SPIFC_TIMING_SMP_CLK_EDGE_SEL_1; ClkEdgeSelTemp1++)
	{
 ...
 */
uint16_t SPIFC_ReadREMS()
{
	uint8_t SpiStatusRegister[2];

  SPIFC_CmdAddrRx(SPIFC_FLASH_REMS, 0x000000, SpiStatusRegister, 2);
 ...
 *
 */
DISABLE_OPT static int8_t check_lib_tag()
{
  uint8_t const volatile *pLibTag;

	pLibTag = T_GPLibTag_0100160F;
	if(pLibTag == NULL)
	{
	  return 0;
	}
...
void SPIFC_Open()
{
  ...
  if((IOFUNC->CTRL0 & GPIOFUNC_CTRL0_SPIFC_IOSEL_MSK) == 0)
	{
	  /*
	   * Hold pin(IOB0):pull-high, WP pin(IOB5):pull low
	   */
		GPIO_SetMode(GPIOB, 0x00000021, GPIO_MODE_INPUT);
		MODIFY_REG(GPIOB->OBUF, 0x00000021, 0x0000001);
	}
	else
	{
	  /*
	   * Hold pin(IOB2):pull-high, WP pin(IOB3):pull low
	   */
		GPIO_SetMode(GPIOB, 0x0000000C, GPIO_MODE_INPUT);
		MODIFY_REG(GPIOB->OBUF, 0x0000000C, 0x0000004);
	}
	SPIFC_SetClkDiv(SPIFC_CLKSEL_HCLK_DIV4);
  SPIFC_EnhanModeReset();
	l_SPIFlashREMS = SPIFC_ReadREMS();
  ...
  SPIFC_Cmd(SPI_FLASH_RDP);                     		// Release DP CMD
  for(l_index = 0; l_index < 512; l_index++);       // Delay 100us
}
 ...
 *   None
 */
void SPIFC_AutoMode(uint32_t MultiIoMode)
{
  ...
  uint8_t l_StatusReg[2];
    ...
    if(mSpiFcQuadModeEnBit != 0)                                               // QE bit exist
    {
      ...
      {
        SPIFC_CmdRx(SPIFC_FLASH_RDSR, l_StatusReg, 2);
      }
      else
      {
        SPIFC_CmdRx(SPIFC_FLASH_RDSR, l_StatusReg, 1);
        SPIFC_CmdRx(mSpiFcReadStatusReg2, &l_StatusReg[1], 1);
      }
      ...
      if((l_temp & l_QEtemp) == 0)
      {
        ...
        {
          SPIFC_CmdTx(SPIFC_FLASH_WRSR, l_StatusReg, 2);
        }
        else
        {
          if(mSpiFcQuadModeEnBit > 7)
					{
            SPIFC_CmdTx(mSpiFcWriteStatusReg2, &l_StatusReg[1], 1);
					}
          else
					{
            SPIFC_CmdTx(mSpiFcWriteStatusReg2, &l_StatusReg[0], 1);
					}
        }
 ...
 *   SPI flash ID
 */
uint32_t SPIFC_ReadID()
{
	uint8_t SpiFlashId[3];

  SPIFC_CmdRx(SPIFC_FLASH_RDID, SpiFlashId, 3);
 ...
 *   None.
 */
void SPIFC_WriteDisable()
{
  SPIFC_Cmd(SPIFC_FLASH_WRDI);
}
