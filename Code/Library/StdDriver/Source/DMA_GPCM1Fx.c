 ...
 *   April 18, 2024
 * @Abstract:
 *    2024.04.18 Confirm whether the DMA is busy before closing the DMA(in DMA_Close()& DMA_Trigger())
 *    2024.04.18 Mark the DMA CLK disabled function.(in DMA_Close())
 *
 **************************************************************************************************/
...
void DMA_Close(DMA_TYPE_DEF *DmaHandle)
{
  ...
  while((DmaHandle->CTRL & DMA_CTRL_DMA_BUSY_FLAG)!=0);
	DmaHandle->CTRL = 0;
  DMA_UnInstallIsrService(DmaHandle);

	/*
	if((DMA0->CTRL == 0) && (DMA1->CTRL == 0) && (DMA2->CTRL == 0))
	{
  ...
  DmaHandle->CTRL &= ~DMA_CTRL_DMA_ENABLE;
    ...
    case DMA_CTRL_DMA_REQSEL_SPI0:
      ...
      break;
		...
		case DMA_CTRL_DMA_REQSEL_UART:
			if(DmaHandle->DSTADR == (uint32_t)&UART->DATA)
			{
				UART->CTRL |= UART_CTRL_TX_DMA_ENABLE;
			}
			else
			{
				UART->CTRL |= UART_CTRL_RX_DMA_ENABLE;
			}
		  break;
...
void DMA_IRQHandler()
{
  if(((DMA_INT->INTSTS & DMA_INTSTS_DMA0_DONE_INT_FLAG) != 0) && ((DMA0->CTRL & DMA_CTRL_DMA_DONE_INT_ENABLE) != 0))
	{
	...
	else if(((DMA_INT->INTSTS & DMA_INTSTS_DMA1_DONE_INT_FLAG) != 0)  && ((DMA1->CTRL & DMA_CTRL_DMA_DONE_INT_ENABLE) != 0))
	{
	...
	else if(((DMA_INT->INTSTS & DMA_INTSTS_DMA2_DONE_INT_FLAG) != 0) && ((DMA2->CTRL & DMA_CTRL_DMA_DONE_INT_ENABLE) != 0))
	{
