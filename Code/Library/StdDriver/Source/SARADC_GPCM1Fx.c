...
void SAR_ADC_SetAdcMultiChSampleTime(uint8_t *AdcChSampleTime, uint32_t AdcChannelNum)
{
	...
	for(iCount = 0; iCount < AdcChannelNum; iCount++)
	{
		if(iCount < 8)
		{
			SAR_ADC->SMP0 |= (AdcChSampleTime[iCount] << (4 * iCount));
		}
		else if((iCount >= 8) && (iCount < 10))
		{
			SAR_ADC->SMP1 |= (AdcChSampleTime[iCount] << (4 * (iCount - 8)));
		}
	}	
}
 ...
 * @param  
 *   PinMask [in]:
 *    - LINE_PAD_IOA17, LINE_PAD_IOA18, LINE_PAD_IOA19, LINE_PAD_IOA20,
 *    - LINE_PAD_IOA21, LINE_PAD_IOA22, LINE_PAD_IOA23, LINE_PAD_IOA24
 * @return 
 *   None.
 */
