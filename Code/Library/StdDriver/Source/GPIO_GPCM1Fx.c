...
void GPIO_SetMode(GPIO_TYPE_DEF *gpio, uint32_t PinMask, uint32_t GpioMode)
{
  ...
  if(gpio == (GPIO_TYPE_DEF *) GPIOA_BASE)
	{
	  IoNum = 25;
	}
  else if(gpio == (GPIO_TYPE_DEF *) GPIOB_BASE)
	{
		IoNum = 6;
	}
	else
 ...
 *      KEYCHG_WAKEUP_EN_IOA24, KEYCHG_WAKEUP_EN_IOB0,  KEYCHG_WAKEUP_EN_IOB1,  KEYCHG_WAKEUP_EN_IOB2,  
...
{	  
	MODIFY_REG(IOFUNC->CTRL2, (GPIOFUNC_CTRL2_EXT0_PINSEL_MSK << (ExtIntNum << 3)), (ExtIntIoSel << (ExtIntNum << 3)));	
